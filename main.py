...
from models import db, User, Rider_kyc, Vendor_kyc, Item, Order, Transaction, Comment, SavedItem
...
# NOTE: this resets on every server restart/redeploy. For production durability,
# consider storing tokens in the database instead.
verification_tokens = {}

# Ensure upload directories exist at startup
os.makedirs(app.config['UPLOAD_FOLDER_ITEM'], exist_ok=True)
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)


def save_file(file):
    """Save an uploaded file with a unique, sanitized filename. Returns the filename or None."""
...
- Amount: ₹{order.amount}
- Cancelled at: {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC
...
def kyc():
    if request.method == 'POST':
        ...
        if not id_proof_file or not photo_file:
            flash("All KYC documents are required!", "danger")
            return redirect(url_for('kyc'))

        id_proof_path = save_file(id_proof_file)
        photo_path = save_file(photo_file)
...
def additems():
    ...
    if request.method == 'POST':
        ...
        new_item = Item(
            itemid=itemId, name=name, context=context, image_url=image_url,
            stock=stock, amount=amount, user_id=user_id, region=region,
            type=item_type, categories=categories
        )
...
def dashboard():
    ...
    categorized_items = {
        category: Item.query.filter(Item.categories.ilike(f"%{category}%"))
        ...
        if 'user_email' in session:
            ...
            if quantity > item.stock:
                ...
                return redirect(url_for(
                    'place_order', item_name=item.name,
                    amount=item.amount * quantity, seller_email=item.seller.email
                ))
        else:
...
def comment_item(item_id):
    ...
    if text:
        new_comment = Comment(item_id=item_id, user_id=user.id, text=text)
...
def wallet_dashboard():
    if 'rider_id' in session:
        ...
        return render_template(
            'wallet.html', user=None, rider=rider,
            transactions=transactions, wallet_balance=rider.wallet_balance
        )
        ...
        return render_template(
            'wallet.html', user=user, rider=None,
            transactions=transactions, wallet_balance=user.wallet_balance
        )
...
def my_orders():
    ...
    for order in raw_orders:
        ...
        item = Item.query.filter_by(name=order.item_name).first()
        image_url = item.image_url if item else "images/default-food.jpg"
...
Your HomeKitchen Team
    ...
    return render_template(
        'rider_profile.html', rider=rider, total_deliveries=total_deliveries,
        monthly_earnings=monthly_earnings, weekly_earnings=weekly_earnings
    )
...
def rider_kyc():
    if request.method == 'POST':
        ...
        if not all([id_proof, license_file, insurance, photo]):
            flash("All KYC documents are required!", "danger")
            return redirect(url_for('rider_kyc'))
        ...
        new_rider = Rider_kyc(
            ...
            id_proof=save_file(id_proof),
            license=save_file(license_file),
            insurance=save_file(insurance),
...
def admin_login():
    if request.method == 'POST':
        ...
        if (name == app.config['ADMIN_USERNAME']
                and app.config['ADMIN_PASSWORD']
...
def assign_order():
    if request.method == 'POST':
        ...
        orders = Order.query.filter(Order.rider_id == None).all()  # noqa: E711
...
def rider_orders(rider_id):
    ...
    for order in orders:
        ...
        if consumer:
            consumer_details = {
                "name": consumer.name, "phone": consumer.phone,
                "location": consumer.address, "email": consumer.email
            }
        ...
        if vendor:
            vendor_details = {
                ...
                "location": vendor.address, "email": vendor.email
            }
...
def update_order_status(order_id, status):
    ...
    if status.lower() == "in-transit":
        ...
        rider_share = total_amount * app.config['RIDER_COMMISSION_RATE']
        platform_share = total_amount * app.config['PLATFORM_COMMISSION_RATE']
        ...
        admin = db.session.get(User, 1)  # Assumes user id 1 is the platform/admin account
        if admin:
...
def complete_order(order_id):
    ...
    rider = db.session.query(User).filter_by(role="rider").first()
    ...
    platform_wallet = db.session.query(User).filter_by(role="admin").first()
    if platform_wallet:
